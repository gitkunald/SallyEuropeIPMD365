...
public class GoldSealReviewStep implements WorkflowStepFunction {
	...
	public void out(WorkflowStepFunctionArguments arg0) {
		logger.info("Inside Out Func GoldSeal PublishXML dec 21 ....");
				...
				for (com.ibm.pim.docstore.Document document : documents) {
					if (document != null && document.getName().equals(dynamicExcelPath)) {
						...
						for (int row = 1; row < sheet.getPhysicalNumberOfRows(); row++)
							createProductXML(ctx, row, sheet, xmlStreamWriter, item); // Function for creating the xml
	...
	private String uploadFileInRealPath(com.ibm.pim.docstore.Document document) {
		...
		String sFileSystemRootPath = docRealPath.substring(0, docRealPath.indexOf(Constants.REF_DOC_DIR));
